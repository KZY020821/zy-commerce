...
import { askEndpointStream } from "catalog-concierge";
...
export function StorefrontAssistant({
  ...
  return (
    <ConciergeWidget
      ...
      // The same reader the <script> embed uses, so the two cannot drift.
      onSendStream={(input) => askEndpointStream("/api/assistant", { message: input.message, path: pathname })}
