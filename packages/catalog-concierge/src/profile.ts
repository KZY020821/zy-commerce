/**
 * Builds a compact, product-type-agnostic summary of a store's catalogue:
...
export function buildCatalogProfile(products: CatalogueProduct[]): CatalogProfile {
  const byCategory = new Map<string, CatalogueProduct[]>();
  for (const r of products) {
    const key = r.category?.slug ?? "uncategorised";
  ...
  return { productCount: products.length, categories };
}
